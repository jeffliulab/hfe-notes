...
如果一开始就认定“问题只在这个人身上”，
那后面的所有分类最后都会变成更精细的责怪。

这也是为什么课件要把 `old view / new view`、
`internal / external`、
`sharp end / blunt end` 串成一条线：
...
`PCA` 指的是 `Perception - Cognition - Action`，
也就是：
...
!!! tip "复习时重点记这几条"
    ...
    - `slip`、
      ...
      `violation` 必须分开看，
      因为后面的改法不同。
    - `old view` 容易把问题收缩成 blame，
      `new view` 更关注系统怎样制造或放大风险。
    - PCA 模型提醒我们：
      同一个表面错误，
      根因可能在感知、
      认知或动作的不同环节。
    - 给错误分类的终点不是分类本身，
      而是找到更有效的 root cause 和 mitigation。
...
??? info "02 Intro to Human Error (1).pptx | 234 text units"
    下载原件: [02 Intro to Human Error (1).pptx](../assets/source_files/ENP111/02 Intro to Human Error (1).pptx)
    映射页面: `human_error_frameworks`
...
??? info "03 HE Frameworks (2).pptx | 209 text units"
    下载原件: [03 HE Frameworks (2).pptx](../assets/source_files/ENP111/03 HE Frameworks (2).pptx)
    映射页面: `human_error_frameworks`
